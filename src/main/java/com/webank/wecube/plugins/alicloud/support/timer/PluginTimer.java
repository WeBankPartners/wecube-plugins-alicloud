...
public interface PluginTimer {
    ...
    /**
     * Run schedule task with default timeout time.
     *
     * @param task runnable task
     */
    static void runScheduleTask(PluginTimerScheduleTask task) {
        runScheduleTask(task, TIMEOUT);
    }
     ...
     * @throws PluginException when handling the runnable task.
     */
    static void runScheduleTask(PluginTimerScheduleTask task, int timeout) throws PluginException {

        logger.info("The plugin is running task with the timeout: [{}]", timeout);

        final ScheduledExecutorService service = Executors.newScheduledThreadPool(1);
        final Future<?> scheduledFuture = service.scheduleAtFixedRate(task, 0, 5, TimeUnit.SECONDS);
        try {
            scheduledFuture.get(timeout, TimeUnit.MINUTES);
        } catch (InterruptedException e) {
            throw new PluginException(String.format("The running task thread has been interrupted. The error is: %s", e.getMessage()));
        } catch (ExecutionException e) {
            if (e.getCause() instanceof InterruptTaskException) {

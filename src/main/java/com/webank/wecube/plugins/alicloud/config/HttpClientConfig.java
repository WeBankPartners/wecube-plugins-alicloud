...
import com.webank.wecube.plugins.alicloud.common.HttpClientProperties;
...
//        template.setInterceptors(Collections.singletonList(restTemplateInterceptor));
    ...
    public static class HttpRequestErrorHandler implements ResponseErrorHandler {
        ...
        @Override
        public void handleError(ClientHttpResponse clientHttpResponse) throws IOException, PluginException {
            if (StringUtils.isEmpty(clientHttpResponse.getBody()) || clientHttpResponse.getStatusCode().isError()) {
                if (clientHttpResponse.getStatusCode().is4xxClientError()) {
                    throw new PluginException(String.format("The target server returned error code: [%s]. The target server doesn't implement the request controller.", clientHttpResponse.getStatusCode().toString()));
                }

                if (clientHttpResponse.getStatusCode().is5xxServerError()) {
                    throw new PluginException(String.format("The target server returned error code: [%s], which is an target server's internal error.", clientHttpResponse.getStatusCode().toString()));
                }
            }
        }
    }
